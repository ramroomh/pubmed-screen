...
def generate_links():
    ...
    print("For example, entering 'Psych*' returns the terms: Psychology, Psychopharmacology.\n")
    print("Use of MeSH terms are encouraged. Please visit https://meshb.nlm.nih.gov/search for more information.")
    ...
    while True:
        ...
        print("Please enter the filepath separated by '/'. The last entry will be the name of your .csv and .jpg files.")
    ...
    # Create URls by substituting keywords
            URL = "https://pubmed.ncbi.nlm.nih.gov/?term=%28%28"+ fill1 + "+AND+%28" + fill2 + "&size=200"
...
def count_duplicates(save_path, df, notebook):
    ...
    df2 = df2.reset_index()
    df2.rename(columns={"index":"Search Number",0:"Number of Non-Duplicates"}, inplace=True)
    df2.insert(loc=2, column="Cum Non-Dup", value=df2["Number of Non-Duplicates"].cumsum())
